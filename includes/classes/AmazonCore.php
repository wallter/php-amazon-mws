...
abstract class AmazonCore
{
     ...
     * @param string       $store_name [optional] <p>Name for the store you want to use as seen in the config file.
     ...
     */
    protected function __construct($store_name, $mock = false, $m = null, $config = null)
    {
        $this->configureStore($store_name, $config);
        $this->setMock($mock, $m);
...
<ErrorResponse xmlns="http://mws.amazonaws.com/doc/2009-01-01/">
  ...
  <RequestID>123</RequestID>
     ...
     */
    public function configureStore($store_name, $config = null)
    {
        if (!empty($store_name)) {
            ...
            if (empty($this->config)) {
                $this->log("Config missing", 'Urgent');
                die('AmazonCore - Missing Config');
            } elseif (!isset($this->config[$store_name])) {
                $this->log("Missing `{$store}` in \$config!", 'Urgent');
                die("AmazonCore - Missing `{$store}` in \$config!");
            }

            $this->storeName = $store_name;
            $this->store_config = $this->config[$store_name];

            if (array_key_exists('merchant_id', $this->store_config)) {
                $this->options['SellerId'] = $this->store_config['merchant_id'];
            } else {
                $this->log("Merchant ID is missing!", 'Warning');
            }
            if (array_key_exists('access_key_id', $this->store_config)) {
                $this->options['AWSAccessKeyId'] = $this->store_config['access_key_id'];
            } else {
                $this->log("Access Key ID is missing!", 'Warning');
            }
            if (array_key_exists('secret_key', $this->store_config)) {
                 $this->log("Secret Key is missing!", 'Warning');
            }
            if (!empty($this->store_config['service_url'])) {
                $this->urlbase = $this->store_config['service_url'];
            }
        } else {
            $this->log("Store name `{$s}` empty!", 'Warning');
        }
    }
    ...
    protected function log($msg, $level = 'Info')
    {
        if ($msg != false) {
            ...
            if (isset($this->store_config['log_funciton']) && !empty($this->store_config['log_funciton']) && function_exists($this->store_config['log_funciton'])) {
                switch ($level) {
                    case ('Info'):
                        $loglevel = LOG_INFO;
                    break;
                ...
                call_user_func($this->store_config['log_funciton'], $msg, $loglevel);
            }

            if (empty($this->config['log_path']) || isset($muteLog) && $muteLog == true) {
                return;
            }
            ...
            if (!file_exists($this->store_config['log_path'])) {
                //attempt to create the file if it does not exist
                file_put_contents($this->store_config['log_path'], "This is the Amazon log, for Amazon classes to use.\n");
            }
            if (file_exists($this->store_config['log_path']) && is_writable($this->store_config['log_path'])) {
                $str = "[$level][" . date("Y/m/d H:i:s") . " $name@$ip $fileName:$line $function] " . $msg;
                $fd = fopen($this->store_config['log_path'], "a+");
                fwrite($fd, $str . "\r\n");
                fclose($fd);
            } else {
                throw new Exception('Error! Cannot write to log! (' . $this->store_config['log_path'] . ')');
            }
        } else {
            return false;
        }
     ...
     */
    protected function genTime($in_time = false)
    {
        $time = (!$in_time) ? strtotime('now') : strtotime($in_time);
    ...
    protected function genQuery()
    {
        ...
        $this->options['Signature'] = $this->_signParameters($this->options, $this->store_config['secret_key']);
    ...
    protected function sendRequest($url, $param)
    {
        ...
        while ($response['code'] !== null && $response['code'] == '503' && $this->throttleStop == false) {
            $this->sleep();
            $response = $this->fetchURL($url, $param);
        }
    ...
    protected function _calculateStringToSignV2(array $parameters)
    {
        $data = 'POST';
        $data.= "\n";
        $endpoint = parse_url($this->getFullURL());
        $data.= $endpoint['host'];
        $data.= "\n";
