 ...
 * - valid()
 * Classes list:
 * - AmazonOrderList extends AmazonOrderCore
 */
...
class AmazonOrderList extends AmazonOrderCore implements Iterator
{
    ...
    public function __construct($s = null, $mock = false, $m = null, $config = null)
    {
        ...
        if (isset($this->store_config) && array_key_exists('marketplace_id', $this->store_config)) {
            $this->options['MarketplaceId.Id.1'] = $this->store_config['marketplace_id'];
        } else {
    ...
    public function getFullURL()
    {
        ...
        if (substr($url, -1) !== '/' && substr($this->urlbranch, 1) !== '/') {
            $url.= '/';
        }

        return $url . $this->urlbranch;
    }
    ...
    public function fetchOrders($r = true)
    {
        ...
        // die('query - in amazonorderlist::fetchOrders()');

        $path = $this->options['Action'] . 'Result';
        if ($this->mockMode) {
            $xml = $this->fetchMockFile()->$path;
        } else {
            $response = $this->sendRequest($url, array(
                'Post' => $query
            ));

            dump($response);
            die('response - in amazonorderlist::fetchOrders()');
